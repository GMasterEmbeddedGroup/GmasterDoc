...
本文参照《RoboMaster 2026 机甲大师高校系列赛通信协议 V2.0.0（20260626）》编写，包含机器人侧串口协议和自定义客户端协议。
...
串口统一使用 **8 位数据位、1 位停止位、无硬件流控、无校验位（8N1）**。
...
通信性能参考：正常情况下延迟约 130 ms、丢包率小于 1%；网络较差时延迟约 200 ms、丢包率约 3%。这些是参考测量值，并非实时性保证。

**实现建议**：接收端先寻找 `0xA5`，收齐帧头并检查 CRC8，再根据长度收齐完整帧并检查 CRC16，最后按命令码解析；同时处理半包、粘包、超长长度和校验失败后的重新同步。CRC 参数见附录一。

### 1.2 命令码 ID 和常规链路数据

长度单位均为 byte，表示数据段长度。标有“上限”的频率为发送限制。
...
**长度待核对**：`0x0203`、`0x0303`、`0x0307` 的标注长度分别为 16、15、103 byte，而对应字段及结构体分别合计为 12、12、105 byte。接入时需核对实际裁判系统数据，确认数据段长度后再解析。

#### 常规状态数据关键字段

| 命令码 | 字段与解析要点 |
| --- | --- |
| `0x0001` | 首字节低 4 位为比赛类型，高 4 位为比赛阶段；偏移 1 为 2 字节阶段剩余秒数，偏移 3 为 8 字节 UNIX 时间，连接裁判系统 NTP 后生效 |
| `0x0002` | 0=平局，1=红方胜，2=蓝方胜 |
| `0x0003` | 10 个 2 字节字段：己方 1、2、3、4 号血量，双方总伤害差，己方 7 号、前哨站、基地血量，对方前哨站、基地血量；伤害差为 `int16_t`，其余为 `uint16_t` |
| `0x0101` | 补给区、能量机关、高地、飞镖命中、中心增益点、堡垒及前哨站增益点等位域 |
| `0x0104` | 判罚等级、违规机器人 ID、对应判罚等级累计次数 |
| `0x0105` | 飞镖发射剩余时间，以及命中目标、累计命中次数、选定目标等位域 |
| `0x0201` | ID、等级、当前/最大血量、热量冷却值、热量上限、底盘功率上限；偏移 12 为 `float bullet_speed_limit`，偏移 16 的 bit 0～2 为云台/底盘/发射机构供电状态 |
| `0x0202` | 前 8 字节为保留位；偏移 8、10、12 分别为 2 字节缓冲能量（J）、17 mm 热量、42 mm 热量 |
| `0x0203` | 包含 `float x`、`float y`（m）和 `float angle`（度，正北为 0） |
| `0x0204` | 回血、热量冷却、防御、负防御、攻击增益，以及剩余能量阈值位图；冷却增益为直接值，其余上述增益为百分比 |
| `0x0206` | 装甲 ID 与扣血原因；属于本地判定，实际是否扣血仍取决于规则 |
| `0x0207` | 弹丸类型、发射机构 ID、射频（Hz）、弹丸初速度（m/s） |
| `0x0208` | 17 mm 允许发弹量、42 mm 允许发弹量、剩余金币、堡垒储备允许发弹量 |
| `0x0209` | RFID 场地交互点状态位图 |
| `0x020A` | 飞镖闸门状态、保留位、切换目标时间、最近确认发射时间 |
| `0x020B` | 己方英雄、工程、3/4 号步兵的 x/y 坐标和保留字段 |
| `0x020C` | bit 0～5 为对方 1/2/3/4/6/7 号标记状态，bit 6～11 为己方对应状态；bit 12～15 为双方空中机器人的激光瞄准与反制状态 |
| `0x020D` | `uint32_t sentry_info`、`uint16_t sentry_info_2`、`uint64_t sentry_info_3`：兑换/复活状态、姿态与强化标记、各姿态剩余时长 |
| `0x020E` | bit 0～1 为双倍易伤机会数，bit 2 为是否生效，bit 3～4 为己方加密等级，bit 5 为能否修改密钥 |

#### 0x0301 机器人交互与 UI

数据段前 6 字节依次为 `uint16_t data_cmd_id`、`uint16_t sender_id`、`uint16_t receiver_id`，之后是最多 112 字节内容。完整包不超过 127 字节，**所有子内容共享 30 Hz 上行频率上限**。
...
UI 单个图形由 3 字节图形名和三组 4 字节配置组成。配置包含操作、图形类型、图层、颜色、线宽、起点及类型相关参数。图层为 0～9；图形名用于修改和删除索引。图形坐标原点在屏幕左下角，右上角为 `(1920, 1080)`；角度 0° 为 12 点钟方向，顺时针绘制。浮点显示值为所传 32 位整型数除以 1000。
...
自定义客户端与机器人交互协议无重传机制。`0x0310` 的 300 byte 与客户端所写的 2.4 kbit 对应；不要将其与下行 `0x0311` 的 30 byte 混用。

### 1.5 非链路数据

`0x0306` 用于自定义控制器模拟键鼠操作选手端，数据长 8 byte，发送频率上限 30 Hz。
...
数据由信号发射源发送至雷达，以下消息均以 10 Hz 持续发送。
...
1. 按照下文的 Protobuf 消息格式编写 `.proto` 文件，用 protoc 生成不同语言的类代码；
2. 在发布者端，将对象序列化成二进制；
...
以下消息定义使用 `optional` 声明可选字段，创建 `.proto` 文件时需添加 `syntax = "proto3";`。消息名同时作为 MQTT Topic，字段编号、类型和方向应与对应定义一致。
...
**V2.0.0 补充**：事件 3 的参数为激活灯臂数和平均环数，例如 `10,9.6`；事件 9 的命中目标还包括 5=基地末端移动目标；事件 15 的结果为 0=成功、1=拔出、2=超时、3=离开装配区过久、4=工程战亡、5=四级协作超时、6=主动退出、7=结算未检测到能量单元、8=强制退出缓冲期到期。事件 2 的前哨站 ID 标注为 11/111，与附录二的 10/110 不一致，接入时需核对。
...
**增量基准待核对**：客户端路径增量定义为相对起始点，串口 `0x0307` 的路径增量定义为相对上一个点。两者描述不一致，转换或绘制路径前需核对实际数据。
...
**外层消息定义待核对**：`repeated uint32 RadarSingleRobotInfo;` 缺少字段编号，且与下方消息结构存在类型歧义，不能直接编译。实现前需确认外层字段的类型和编号。
...
CRC8 的生成多项式为 `x^8 + x^5 + x^4 + 1`。实现时需确保查表运算、初值和收尾方式均符合协议，不能仅凭 CRC 算法名称判断是否兼容。

推荐验证顺序：先调用帧头 CRC8 校验，再按 `data_length + 9` 收齐帧并校验 CRC16，通过后解析命令数据。移植校验代码时需检查类型拼写、标点和校验结果。
...
下列示例依赖 Unity、MQTTnet 及项目自己的 `Debugger`、`MqttSubProtoEvent`、`ThreadPool`，并非独立可运行程序。发送协议消息应使用 Protobuf 序列化后的字节数组和对应 Topic。示例默认连接 `192.168.12.1`，使用 `AtLeastOnce`（QoS 1），与第 2 章的连接参数及最高 QoS 一致。
