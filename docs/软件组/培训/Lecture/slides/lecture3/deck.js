...
(function () {
  ...
  function initQuiz(root) {
    $$('.quiz', root).forEach(function (box) {
      var opts = $$('.quiz-opt', box);
      var feedback = $('.quiz-feedback', box);
      ...
      opts.forEach(function (opt) {
        opt.addEventListener('click', function () {
          ...
          var right = opt.dataset.correct === '1';
          opts.forEach(function (o) {
            o.classList.add('is-locked');
            if (o.dataset.correct === '1') { o.classList.add('is-right'); }
          });
          if (!right) { opt.classList.add('is-wrong'); }

          if (feedback) {
            var text = opt.dataset.explain ||
              (right ? '正确。' : '再看一下其它选项，正确答案已在上面标出。');
            feedback.innerHTML = (right ? '<b>✔ 正确。</b> ' : '<b>✘ 再想想。</b> ') + text;
